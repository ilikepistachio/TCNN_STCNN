...
class thumos14():
  ...
  def _load_annotations(self, video_prefix, num_frames):
    ...
    filename = os.path.join(
      self._data_path, 'PNGImages', video_prefix,'0000001.png')
  ...
  def next_val_video(self, curr_idx, random=False):
    video = self.vddb[curr_idx]
    ...
    _, vid = self.clip_reader(video['video_prefix'], video['num_frames'])

    return vid - self._mean_frame,\
           video['gt_labels'][0],\
           video['gt_bboxes'],\
           video['gt_length'],\
           is_last
